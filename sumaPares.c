...
int main()
{
    ...
    // 'suma' es una variable que acumulará el total de la suma de los números pares. Se inicializa en 0.
    ...
    while(contador <= max)
    {
        ...
        // Esto nos sirve para determinar si el número es par.
        residuo = contador % 2;

        // ---- CONDICIONAL IF/ELSE ----
        // Esta estructura revisa si el número actual del contador es par.

        // Comparamos si el valor de 'residuo' es exactamente igual a 0.
        // Si la condición se cumple, significa que 'contador' es un número par.
        if(residuo == 0)
        {
            // Si el número es par, se muestra en pantalla para indicar que se está sumando.
            // Se imprime el número ('%d') seguido de un signo de suma.
            printf("%d+", contador);
            // El valor del contador (que es par) se añade a la variable 'suma'.
            suma = suma + contador;
        }
        // Si la condición del 'if' no se cumple.
        else
        {
            // Este bloque 'else' se ejecuta si el número no es par (es impar).
    ...
    printf("\nLa suma de los numeros pares listados es: %d", suma);
